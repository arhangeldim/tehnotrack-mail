...
* Сервер должен поддерживать 2 режима работы (старый код не нужно удалять) - ServerSocket (io)/ ServerSocketChannel (nio). То есть нужно абстрагироваться от механизма
чтения данных по сети. 
...
* Изучите примеры с занятия по nio. Напишите свой nio сервер, который слушает селектор в одном потоке,
получает сигналы с него и раздает данные потокам обработчикам.
